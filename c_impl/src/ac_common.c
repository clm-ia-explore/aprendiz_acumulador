 ...
 * Formato compatible con la versión Python
 */
...
// Magic strings (deben coincidir con Python)
static const char MAGIC_SHM[8] = "ACCSHM01";
static const char MAGIC_DAT[8] = "ACCDAT01";
static const char MAGIC_QDAT[8] = "ACCQDAT1";

// Tamaños de cabecera (en bytes)
#define SHM_HEADER_SIZE 24  // 8sIIII = 8+4+4+4+4
#define DAT_HEADER_SIZE 24  // 8sIIII = 8+4+4+4+4
#define QDAT_HEADER_SIZE 32 // 8sIIIiiI = 8+4+4+4+4+4+4
...
const char* ac_get_data_dir(void) {
    const char *dir = getenv("ACC_SHM_DIR");
    return dir ? dir : ".";
}

int ac_build_path(char *buf, size_t bufsize, const char *name, const char *ext) {
    const char *dir = ac_get_data_dir();
    int ret = snprintf(buf, bufsize, "%s/acc_%s%s", dir, name, ext);
    if (ret < 0 || (size_t)ret >= bufsize) return -1;
    return 0;
}
...
int ac_read_header(const char *filepath, ac_bin_header_t *header) {
    ...
    if (!f) return -1;
    size_t rd = fread(header, sizeof(ac_bin_header_t), 1, f);
    fclose(f);
    if (rd != 1) return -1;
    if (memcmp(header->magic, MAGIC_SHM, 8) != 0) return -2;
...
int ac_write_header(const char *filepath, const ac_bin_header_t *header) {
    ...
    if (!f) return -1;
    size_t wr = fwrite(header, sizeof(ac_bin_header_t), 1, f);
    fclose(f);
    return (wr == 1) ? 0 : -1;
}
...
int ac_read_metadata(const char *name, ac_meta_cache_t *meta) {
    char path[512];
    if (ac_build_meta_path(path, sizeof(path), name) != 0) return -1;
    FILE *f = fopen(path, "rb");
    if (!f) return -1;
    size_t rd = fread(meta, sizeof(ac_meta_cache_t), 1, f);
    fclose(f);
    return (rd == 1) ? 0 : -1;
}
...
int ac_write_metadata(const char *name, const ac_meta_cache_t *meta) {
    ...
    if (!f) return -1;
    fwrite(meta, sizeof(ac_meta_cache_t), 1, f);
    fclose(f);
    if (rename(temp, path) != 0) { unlink(temp); return -1; }
    return 0;
}

int ac_validate_file(const char *filepath) {
    ac_bin_header_t header;
    return ac_read_header(filepath, &header);
}
...
void* ac_mmap_file(const char *filepath, size_t *size, int *fd, int writable) {
    ...
    *fd = open(filepath, writable ? O_RDWR : O_RDONLY);
    ...
    if (fstat(*fd, &st) < 0) { close(*fd); return MAP_FAILED; }
    *size = st.st_size;
    void *addr = mmap(NULL, *size, writable ? (PROT_READ | PROT_WRITE) : PROT_READ, MAP_SHARED, *fd, 0);
...
uint32_t ac_simple_checksum(const void *data, size_t len) {
    ...
    uint32_t sum = 0;
    for (size_t i = 0; i < len; i++) sum = (sum << 1) ^ bytes[i];
    return sum;
}
...
int ac_normalize_values(const double *input, double *output, uint64_t n,
    ...
    if (method == NORM_MINMAX) {
        ...
        double rng = mx - mn;
        if (rng == 0) { for (uint64_t i = 0; i < n; i++) output[i] = 0.0; }
        else { for (uint64_t i = 0; i < n; i++) output[i] = (input[i] - mn) / rng; }
        return 0;
    }
    
    if (method == NORM_POSMAX) {
        double mx = 0.0;
        for (uint64_t i = 0; i < n; i++) { double v = input[i] > 0 ? input[i] : 0; if (v > mx) mx = v; }
        if (mx <= 0) { for (uint64_t i = 0; i < n; i++) output[i] = 0.0; }
        else { for (uint64_t i = 0; i < n; i++) output[i] = ((input[i] > 0 ? input[i] : 0) / mx); }
        return 0;
    }
    
    if (method == NORM_SIGNED01) {
        double mx = 0.0;
        for (uint64_t i = 0; i < n; i++) { double a = fabs(input[i]); if (a > mx) mx = a; }
        if (mx <= 0) { for (uint64_t i = 0; i < n; i++) output[i] = 0.5; }
        else { for (uint64_t i = 0; i < n; i++) output[i] = (input[i] / mx + 1.0) / 2.0; }
        return 0;
    }
    ...
    if (method == NORM_SUM01) {
        ...
        if (sum <= 0) { for (uint64_t i = 0; i < n; i++) output[i] = 1.0 / n; }
        else { for (uint64_t i = 0; i < n; i++) output[i] = (input[i] - mn) / sum; }
        return 0;
    }
    ...
    if (method == NORM_SOFTMAX) {
        ...
        double sum = 0;
        for (uint64_t i = 0; i < n; i++) { output[i] = exp((input[i] - mx) / temperature); sum += output[i]; }
        if (sum <= 0) { for (uint64_t i = 0; i < n; i++) output[i] = 1.0 / n; }
        else { for (uint64_t i = 0; i < n; i++) output[i] /= sum; }
        return 0;
    }
    ...
    if (method == NORM_SIGMOID) {
        ...
        for (uint64_t i = 0; i < n; i++) {
            ...
            if (z >= 0) output[i] = 1.0 / (1.0 + exp(-z));
            else { double e = exp(z); output[i] = e / (1.0 + e); }
        }
        return 0;
    }
    ...
    if (method == NORM_TANH01) {
        if (scale <= 0) { ac_error("scale must be positive"); return -1; }
        for (uint64_t i = 0; i < n; i++) output[i] = (tanh(input[i] / scale) + 1.0) / 2.0;
        return 0;
    }
        ...
        for (uint64_t i = 0; i < n - 1; i++) {
            for (uint64_t j = i + 1; j < n; j++) {
                if (input[indices[i]] > input[indices[j]]) {
                    uint64_t tmp = indices[i]; indices[i] = indices[j]; indices[j] = tmp;
                }
            }
        }
...
int ac_quantize_floats(const double *floats, int32_t *output, uint64_t n,
    ...
    if (renorm_max) {
        ...
        if (mx > 0) { for (uint64_t i = 0; i < n; i++) vals[i] /= mx; }
        else { for (uint64_t i = 0; i < n; i++) vals[i] = 0; }
    }
...
int ac_read_dat_header(const char *filepath, ac_dat_header_t *header) {
    ...
    fclose(f);
    if (rd != 1) return -1;
    if (memcmp(header->magic, MAGIC_DAT, 8) != 0) return -2;
    if (header->version != AC_VERSION) return -3;
    return 0;
}
...
int ac_read_dat_values(const char *filepath, int64_t **values, uint64_t *n) {
    ...
    if (ac_read_dat_header(filepath, &header) != 0) { ac_error("archivo .dat inválido"); return -1; }
    FILE *f = fopen(filepath, "rb");
    if (!f) return -1;
    fseek(f, DAT_HEADER_SIZE, SEEK_SET);
    *n = header.n;
    *values = malloc(header.n * sizeof(int64_t));
    if (!*values) { fclose(f); ac_error("out of memory"); return -1; }
    size_t rd = fread(*values, sizeof(int64_t), header.n, f);
    fclose(f);
    if (rd != header.n) { free(*values); ac_error("lectura incompleta"); return -1; }
    uint32_t calc_crc = ac_crc32(*values, header.n * sizeof(int64_t));
...
int ac_write_dat_file(const char *filepath, uint64_t n, const int64_t *values, int force) {
    if (!force && access(filepath, F_OK) == 0) { ac_error("el archivo ya existe"); return -1; }
    ac_dat_header_t header;
    memcpy(header.magic, MAGIC_DAT, 8);
...
int ac_read_qdat_header(const char *filepath, ac_qdat_header_t *header) {
    ...
    fclose(f);
    if (rd != 1) return -1;
    if (memcmp(header->magic, MAGIC_QDAT, 8) != 0) return -2;
    if (header->version != AC_VERSION) return -3;
    return 0;
}
...
int ac_read_qdat_values(const char *filepath, int32_t **values, uint64_t *n,
    ...
    if (ac_read_qdat_header(filepath, &header) != 0) { ac_error("archivo .qdat inválido"); return -1; }
    FILE *f = fopen(filepath, "rb");
    if (!f) return -1;
    fseek(f, QDAT_HEADER_SIZE, SEEK_SET);
    *n = header.n;
    *values = malloc(header.n * sizeof(int32_t));
    if (!*values) { fclose(f); ac_error("out of memory"); return -1; }
    size_t rd = fread(*values, sizeof(int32_t), header.n, f);
    fclose(f);
    if (rd != header.n) { free(*values); ac_error("lectura incompleta"); return -1; }
    uint32_t calc_crc = ac_crc32(*values, header.n * sizeof(int32_t));
    if (calc_crc != header.crc32) { free(*values); ac_error("CRC32 incorrecto"); return -1; }
...
int ac_write_qdat_file(const char *filepath, uint64_t n, ac_norm_method_t method,
    ...
    if (!force && access(filepath, F_OK) == 0) { ac_error("el archivo ya existe"); return -1; }
    ac_qdat_header_t header;
    memcpy(header.magic, MAGIC_QDAT, 8);
    header.version = AC_VERSION;
    header.n = (uint32_t)n;
