...
int ac_init_main(int argc, char *argv[]) {
    ...
    // Escribir archivo con cabecera + datos inicializados a 0
    ...
    // Escribir cabecera
    if (fwrite(&header, sizeof(ac_header_t), 1, f) != 1) {
    ...
    ac_info("Tamaño en disco: ~%zu bytes", sizeof(ac_header_t) + data_size);
    return 0;
}
